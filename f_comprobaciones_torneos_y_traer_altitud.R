f_comprobaciones_torneos_y_traer_altitud <- function(data){
  ...
  torneos_tabla_maestra <- try(
    read.csv( paste0("C:/Users/Carlos/Google Drive/tenis_pfp/pfp/tablas/torneos_tabla_maestra.csv"), 
              header=T , 
              dec=".",
              sep=';', 
              encoding = "UTF-8",
    ...
    silent = TRUE)
  if("try-error" %in% class(torneos_tabla_maestra))
    torneos_tabla_maestra <- read.csv( paste0("C:/Users/cvaliente/Google Drive/tenis_pfp/pfp/tablas/torneos_tabla_maestra.csv"), 
                                       header=T , 
                                       dec=".",
                                       sep=';', 
                                       encoding = "UTF-8",
  ...
  ################ COMPROBACIONES #################### OBLIGATORIO EJECUTAR PORQUE CREA data2
    ...
    e <- print(pru[duplicated(pru$tourney_id_and_draw),])
    if(nrow(e)>0){
    ...
    print(data2 %>% dplyr::filter(id_tourney_NO_year %in% pru[duplicated(pru$id_tourney_NO_year),"id_tourney_NO_year"]) %>%
